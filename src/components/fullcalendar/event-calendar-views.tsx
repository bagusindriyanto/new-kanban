...
const dayRowCommonClasses: CalendarOptions = {
  ...
  listItemEventClass: (info) =>
    cn('mb-px p-px rounded-sm', info.isNarrow ? 'mx-px' : 'mx-0.5'),
  listItemEventBeforeClass: (info) =>
    cn('border-4', info.isNarrow ? 'ms-0.5' : 'ms-1'),
  ...
  rowMoreLinkInnerClass: (info) =>
    info.isNarrow ? `px-0.5 py-px ${xxsTextClass}` : 'px-1 py-0.5 text-xs',
};
  ...
  return (
    <FullCalendar
      ...
      backgroundEventTitleClass={(info) =>
        cn(
          'opacity-50 italic',
          info.isNarrow ? `px-1 py-1.5 ${xxsTextClass}` : 'px-2 py-2.5 text-xs',
        )
      }
      /* List-Item Event
      ----------------------------------------------------------------------------------------- */
      ...
      columnMoreLinkInnerClass={(info) =>
        info.isNarrow ? `p-0.5 ${xxsTextClass}` : 'p-1 text-xs'
      }
      /* Day Header
      ----------------------------------------------------------------------------------------- */
          ...
          {info.dayNumberText && (
            <div
              className={cn(
                ...
                info.isNarrow ? 'size-7 text-base' : 'size-8 text-lg',
                info.isToday
                  ? [
                      'bg-primary text-primary-foreground',
                      info.hasNavLink && 'group-hover:bg-primary/80',
                    ]
                  : info.hasNavLink && 'group-hover:bg-foreground/5',
                info.hasNavLink &&
                  'group-focus-visible:outline-3 outline-ring/50',
              )}
            >
              {info.dayNumberText}
            </div>
      ...
      dayCellTopClass={(info) =>
        cn(
          'flex flex-row',
          info.isNarrow ? 'justify-end min-h-px' : 'justify-center min-h-0.5',
        )
      }
      dayCellTopInnerClass={(info) =>
        cn(
          'flex flex-row items-center justify-center whitespace-nowrap rounded-full',
          info.isNarrow ? `m-px h-5 ${xxsTextClass}` : 'm-1.5 h-6 text-sm',
          ...
          info.isToday
            ? [
                'bg-primary text-primary-foreground',
                info.hasNavLink &&
                  'hover:bg-primary/80 focus-visible:outline-3 outline-ring/50',
              ]
      ...
      listDayClass={(info) =>
        cn(!info.isLast && 'border-b', 'flex flex-row items-start')
      }
      ...
      fillerClass={(info) =>
        cn('opacity-50 border', info.inTableHeader && 'border-transparent')
      }
      dayNarrowWidth={100}
      ...
      inlineWeekNumberClass={(info) =>
        cn(
          ...
          info.hasNavLink &&
            'hover:bg-foreground/20 focus-visible:outline-3 outline-ring/50',
        )
      }
        ...
        timeGrid: {
          ...
          weekNumberHeaderInnerClass: (info) =>
            cn(
              ...
              info.isNarrow ? 'h-5 px-1.5 text-xs' : 'h-6 px-2 text-sm',
              info.hasNavLink &&
                'hover:bg-foreground/20 focus-visible:outline-3 outline-ring/50',
            ),
          ...
          slotHeaderDividerClass: (info) =>
            cn(
              'border-e',
              info.inTableHeader &&
                info.options.dayMinWidth === undefined &&
                'border-transparent',
            ),
