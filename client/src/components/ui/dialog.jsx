...
function DialogOverlay({ className, ...props }) {
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        'isolate fixed inset-0 z-50 duration-100 bg-black/10 supports-backdrop-filter:backdrop-blur-xs data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0',
        className,
      )}
      {...props}
    />
...
function DialogTitle({ className, ...props }) {
  return (
    <DialogPrimitive.Title
      ...
      className={cn('text-lg font-medium leading-none font-heading', className)}
      {...props}
    />
  );
}
